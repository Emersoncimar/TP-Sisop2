...
void send_from_file(const char *filename) {
...
int main(int argc, char *argv[]) {
    if (argc < 2 || argc > 3) {
        fprintf(stderr, "Uso: %s <porta> [arquivo_com_numeros.txt]\n", argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    if (argc == 3) {
        // Modo automático com arquivo
        send_from_file(argv[2]);
    } else {
        // Modo interativo com stdin
        pthread_t thread;
